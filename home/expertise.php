<div class="cotainer-fluid generic-container">
    <div class="row">
        <div class="col-sm-12 text-center">
            <h2> Our Services/ Expertise</h2>

            <p class="emphase-introduction">
                Engineering solutions for Engineers by Engineers!
            </p>

        </div>
    </div>
    <div class="row px-auto  justify-content-center">

        <a href="expertise/design-and-survey/" class="service-container  col-lg-3 col-md-4 col-sm-6 col-xm-12">
            <img src="img/design&budgeting.jpg" alt="training" class="service-img" />
            <div class="service-title">
                <h3>Design and Survey</h3>
            </div>
            <div class="service-cover">
                <div class="service-cover-title">
                    <h3>Design and Survey</h3>
                </div>
                <div class="service-cover-text">
                    <p>Emeran designers & surveyors are trained to collect the data required on field for electrification projects,
                        they’re trained on... </p>
                </div>
                <div class="service-cover-link">
                    Learn more <i class="fa fa-long-arrow-right"></i>
                </div>
            </div>
        </a>
        <a href="expertise/lv-and-mv-construction/" class="service-container  col-lg-3 col-md-4 col-sm-6 col-xm-12">
            <img src="img/construction.png" alt="training" class="service-img" />
            <div class="service-title">
                <h3>Lv & Mv Construction</h3>
            </div>
            <div class="service-cover">
                <div class="service-cover-title">
                    <h3>Lv & Mv Construction</h3>
                </div>
                <div class="service-cover-text">
                    <p> Emeran is proud to have experienced engineers passionate on their work, well trained site supervisors to ensure...</p>
                </div>
                <div class="service-cover-link">
                    Learn more <i class="fa fa-long-arrow-right"></i>
                </div>
            </div>
        </a>
        <a href="expertise/project-management/" class="service-container  col-lg-3 col-md-4 col-sm-6 col-xm-12">
            <img src="img/project_management.jpg" alt="training" class="service-img" />
            <div class="service-title">
                <h3>Project Management</h3>
            </div>
            <div class="service-cover">
                <div class="service-cover-title">
                    <h3>Project Management</h3>
                </div>
                <div class="service-cover-text">
                    <p>
                        Electrification is Our heart beat. whether you're an individual power utility company or a contractor,
                        large scale or small scale, count on us...
                    </p>
                </div>
                <div class="service-cover-link">
                    Learn more <i class="fa fa-long-arrow-right"></i>
                </div>
            </div>
        </a>
        <a href="../expertise/staffing-and-training/" class="service-container  col-lg-3 col-md-4 col-sm-6 col-xm-12">
            <img src="img/Emeran_open_page.png" alt="training" class="service-img" />
            <div class="service-title">
                <h3>Staffing & Training</h3>
            </div>
            <div class="service-cover">
                <div class="service-cover-title">
                    <h3>Staffing & Training</h3>
                </div>
                <div class="service-cover-text">
                    <p>Emeran has an extensive and multidisplinary team of electrical engineers and technitians
                        who can be outcasted for quick short-term work or complex long-term projects.</p>
                </div>
                <div class="service-cover-link">
                    Learn more <i class="fa fa-long-arrow-right"></i>
                </div>
            </div>
        </a>
    </div>
</div>