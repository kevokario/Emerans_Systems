<nav class="mynav navbar-light">
    <a class="mynav-brand" href="../../"><img src="../../img/logo.png" alt="Emerans" class="mynav-logo"></a>
    <ul class="mynav-links">
        <li>
            <a id="linksA" href="../../expertise/" class="mynav-link">
                Expertise 
                <i class="mynav-caret pull-right fa fa-caret-down"></i>
            </a>
            <div class="menu-options-multiple custom-menu-option text-dark">
                <div class="mymenu">
                    <a class="menu-option-title" href="../design-and-survey/">
                        Design & Survey
                    </a>
                    <div class="menu-option-body">
                        <a href="../design-and-survey/autocard-design.php">Autocard Design</a>
                        <a href="../design-and-survey/costing.php">Costing</a>
                        <a href="../design-and-survey/design-and-survey.php">Design&Survey</a>
                        <a href="../design-and-survey/field-survey.php">Field Survey</a>
                        <a href="../design-and-survey/field-database.php">Field Database</a>
                    </div>
                </div>
                <div class="mymenu">
                    <a class="menu-option-title" href="../lv-and-mv-construction/">
                        Lv & Mv Construction
                    </a>
                    <div class="menu-option-body">
                        <a href="../lv-and-mv-construction/construction-standards.php">Construction Standards</a>
                        <a href="../lv-and-mv-construction/material-requisition.php">Material Requisition</a>
                        <a href="../lv-and-mv-construction/matering.php">Metering</a>
                        <a href="../lv-and-mv-construction/pegging.php">Pegging</a>
                    </div>
                </div>
                <div class="mymenu">
                    <a class="menu-option-title" href="../project-management/">
                        Project Management
                    </a>
                    <div class="menu-option-body">
                        <a href="../project-management/back-office.php">Back Office</a>
                        <a href="../project-management/electrification-systems.php">Electrification System</a>
                        <a href="../project-management/project-management.php">project-management</a>
                        <a href="../project-management/supervision-team.php">Supervision Team</a>
                    </div>
                </div>
                <div class="mymenu">
                    <a class="menu-option-title" href="../staffing-and-training/">
                        Staffing and Training
                    </a>
                    <div class="menu-option-body">
                        <a href="../staffing-and-training/autocard.php">Auto Card</a>
                        <a href="../staffing-and-training/back-office.php">Back Office</a>
                        <a href="../staffing-and-training/construction-teams.php">Construction Teams</a>
                        <a href="../staffing-and-training/staffing-and-training.php">staffing-and-training</a>
                        <a href="../staffing-and-training/supervisors.php">Supervisors</a>
                        <a href="../staffing-and-training/surveyors-and-designers.php">Surveyors and Designers</a>
                    </div>
                </div>
            </div>
        </li>
        <li>
            <a href="../../electrification-systems" id="linksB" class="mynav-link">
                Electrification Systems
                <i class="mynav-caret pull-right fa fa-caret-down"></i>
            </a>
            <div class="menu-options custom-menu-option text-dark">
                <div class="menu-option-body">
                    <a href="../../electrification-systems/ElectrificationSystemsInformation/#budgeting">Design</a>
                    <a href="../../electrification-systems/ElectrificationSystemsInformation/#StoreManagement">Store management</a>
                    <a href="../../electrification-systems/ElectrificationSystemsInformation/#WorkProgress">Work progress</a>
                    <a href="../../electrification-systems/ElectrificationSystemsInformation/#Reporting">Reporting</a>
                    <a href="../../electrification-systems/ElectrificationSystemsInformation/#GanttChart">Gantt chart</a>
                    <a href="../../electrification-systems/ElectrificationSystemsInformation/#MaterialReconciliation">Material reconciliation</a>
                </div>
            </div>
        </li>
        <li>
            <a href="../../projects/" id="linksC" class="mynav-link">
                Projects 
                <i class="mynav-caret pull-right  fa fa-caret-down"></i>
            </a>
            <div class="menu-options custom-menu-option text-dark">
                <div class="menu-option-body">
                    <a>Design & Salary</a>
                    <a>Electrification Management Systems</a>
                    <a>Training</a>
                    <a>Consultance</a>
                </div>
            </div>
        </li>
        <li>
            <a href="#" id="linksD" class="mynav-link">
                Blog
                <!-- <i class="mynav-caret pull-right fa fa-caret-down"></i> -->
            </a>
            <!-- <div class="menu-options custom-menu-option text-dark">
                    <div class="menu-option-body">
                                    <a>sublink a</a>
                                    <a>sublink b</a>
                                    <a>sublink c</a>
                                    <a>sublink d</a>
                            </div>
            </div> -->
        </li>
        <li>
            <a href="../../contact-us.php" id="linksE" class="mynav-link">
                Contact us 
                <!-- <i class="mynav-caret pull-right fa fa-caret-down"></i> -->
            </a>
            <!-- <div class="menu-options custom-menu-option text-dark">
                    <div class="menu-option-body">
                                    <a>sublink a</a>
                                    <a>sublink b</a>
                                    <a>sublink c</a>
                                    <a>sublink d</a>
                            </div>
            </div> -->
        </li>
    </ul>
    <button class="mynav-btn btn pull-right">
        <i class="fa fa-reorder"></i>
    </button>
</nav>
